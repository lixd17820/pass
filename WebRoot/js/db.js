...
function CouchDB(name, httpHeaders) {
	this.name = name;
	this.uri = "http://localhost:5984/" + encodeURIComponent(name) + "/";
	...
	this.open = function(docId, options) {
		this.last_req = this.request("GET", this.uri
				+ encodeURIComponent(docId) + encodeOptions(options));
		if (this.last_req.status == 404)
			return null;
		CouchDB.maybeThrowError(this.last_req);
		return JSON.parse(this.last_req.responseText);
	...
	this.deleteDoc = function(doc) {
		this.last_req = this.request("DELETE", this.uri
				+ encodeURIComponent(doc._id) + "?rev=" + doc._rev);
		CouchDB.maybeThrowError(this.last_req);
		var result = JSON.parse(this.last_req.responseText);
	...
	this.deleteDocAttachment = function(doc, attachment_name) {
		this.last_req = this.request("DELETE", this.uri
				+ encodeURIComponent(doc._id) + "/" + attachment_name + "?rev="
				+ doc._rev);
		CouchDB.maybeThrowError(this.last_req);
		var result = JSON.parse(this.last_req.responseText);
		doc._rev = result.rev; // record rev in input document
	...
	this.bulkSave = function(docs, options) {
		...
		if (this.last_req.status == 417) {
			return {
				errors : JSON.parse(this.last_req.responseText)
			};
		} else {
			CouchDB.maybeThrowError(this.last_req);
	...
	// Applies the map function to the contents of database and returns the
	// results.
	...
	this.allDocsBySeq = function(options, keys) {
		var req = null;
		if (!keys) {
			req = this.request("GET", this.uri + "_all_docs_by_seq"
					+ encodeOptions(options));
		} else {
			req = this.request("POST", this.uri + "_all_docs_by_seq"
					+ encodeOptions(options), {
				headers : {
					"Content-Type" : "application/json"
				},
...
CouchDB.updateUser = function(username, email, roles, password, old_password) {
	...
	CouchDB.last_req = CouchDB.request("PUT", "/_user/"
...
CouchDB.newUuids = function(n) {
	if (CouchDB.uuids_cache.length >= n) {
		...
		if (CouchDB.uuids_cache.length - n == 0) {
			...
			CouchDB.uuids_cache = CouchDB.uuids_cache.slice(0,
					CouchDB.uuids_cache.length - n);
		}
		return uuids;
	} else {
		...
		CouchDB.uuids_cache = CouchDB.uuids_cache.concat(result.uuids.slice(0,
				100));
